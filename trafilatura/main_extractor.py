...
def handle_titles(element: _Element, options: Extractor) -> Optional[_Element]:
    ...
    # children
        ...
        for child in list(element):
            ...
            processed_child = handle_textnode(
                child, options, comments_fix=False
            )
    ...
    if (
        title is not None
        and text_chars_test("".join(title.itertext())) is True
    ):
        return title
    return None
    ...
    for subelem in child.iterdescendants("*"):
        if subelem.tag == "list":
            ...
            processed_subchild = handle_textnode(
                subelem, options, comments_fix=False
            )
            if processed_subchild is not None:
...
def is_text_element(elem: _Element) -> bool:
    ...
    return (
        elem is not None and text_chars_test("".join(elem.itertext())) is True
    )
        ...
        if len(child) == 0:
            ...
            if child.tail is not None and child.tail.strip():
                ...
                if new_child_elem_children:
                    ...
                    if (
                        last_subchild.tail is None
                        or not last_subchild.tail.strip()
                    ):
    ...
    for expr in BODY_XPATH:
        ...
        result_body.extend(
            [
                el
                for el in (
                    handle_textelem(e, potential_tags, options)
                    for e in subelems
                )
                if el is not None
            ]
